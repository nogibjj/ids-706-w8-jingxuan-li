...
fn main() -> Result<()> {
    ...
    println!("Memory used before: {} KB", memory_before);
    println!("Memory used after: {} KB", memory_after);
    let duration = start.elapsed();
    println!("Memory difference: {} KB", memory_after - memory_before);
    println!("Command executed in: {:.2}", duration.as_secs_f64());
    Ok(())
}
